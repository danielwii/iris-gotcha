...
- `0.12.3` — **parse the YAML, don't slice the line**: Step 8 said to "read `index_cue` from frontmatter" without saying what reading means, so a double-quoted cue containing an escaped quote was taken as the text between the first and last `"` on the line. One entry in a 35-entry notebook hit it: `index_cue: "…source 必须写字符串 \"./\""` reached the index as `…source 必须写字符串 \"./\` — escapes survived, a trailing character was eaten, and the closing `**` was left unpaired because `\*` reads as an escaped asterisk. The entry existed to say the value must be the string `"./"`, and the index destroyed exactly that. Step 1 now requires decoding the YAML scalar before the value reaches the render template, and Step 8's validation table gains a mechanically checkable row — rendered lines must carry no `\"` or `\*` and must have paired `**` — so the parsing rule has something that can fail rather than being an instruction alone. A failure there is a defect in this step: re-read the entry as YAML and re-render; never patch the rendered line, never edit the entry to dodge the quote.
- `0.12.2` — **the keyword budget belongs to the rendered line, not to the entry**: Step 8 contradicted itself — step 1 read `keywords` "first 5 only" (a render-time truncation) while step 4 validated "each entry's keyword count ≤ 5" as a write failure. Only one can be operative. On the user-scope notebook, 26 of 29 entries stored more than 5 keywords (largest: 22), so the refusing reading meant no index could be written until keywords were deleted from most of the notebook. Measurement settled it: rendering every stored keyword puts 12 of 30 lines over the 220-code-point line budget (longest 532) — structurally unwritable — while rendering the first 5 puts 0 of 30 over. Truncation is the *precondition* for the line budget being satisfiable, not an alternative to it; and since both readings yield a byte-identical index, enforcing the stored count would have meant irreversibly deleting real search terms from 26 entries to obtain exactly what truncation already produces. The keyword budget now bounds the **rendered** line and cannot fail; entries store as many keywords as retrieval warrants, ordered most-distinctive-first. Fourth defect of this shape in v0.12.0's budget machinery — common root: the budget table gave numbers without saying which artifact each governs (stored entry / rendered line / finished file). Spec-only.
- `0.12.1` — **enforce the mandatory field**: v0.12.0 declared `index_cue` mandatory but no step checked for it — Step 8's validation table was budgets-only, and `action=audit`'s Applicability/Atomicity checks read the field while presupposing it exists. The gap is invisible by construction: a **missing** field trivially satisfies every budget, so a budget-only pass reports "no violations" on a notebook where no entry has the field. Found by auditing the user-scope notebook on 2026-09-05 — 28 of 30 entries had no `index_cue`, two had no conforming frontmatter (one had no frontmatter block at all), and the audit flagged none of it. Step 8 now checks schema presence *before* budgets and refuses to write on a missing frontmatter block / `index_cue` / `type` / `keywords`, with synthesizing a missing cue from the entry body explicitly forbidden (Step 8 doesn't read bodies by design; improvising there reintroduces the unbudgeted prose `index_cue` exists to exclude). `action=audit` runs schema completeness first and must report Applicability/Atomicity as `blocked` — never passing — on entries lacking the field, and must state how many entries each check actually ran against. Also disambiguates budget units: all budgets are **Unicode code points**, never bytes (the whole-file row previously said "characters" while every other row said "code points" — for the notebook that exposed this, the two readings disagree on the verdict: 26,302 code points vs 34,363 bytes against a 32,000 hard budget), and documents the whole-file budget as an **entry-count backstop** rather than a verbosity backstop, since the per-line budget binds first (~145 entries before the file budget can bind). Spec-only; no schema change, no migration.
