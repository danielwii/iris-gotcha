...
- **`index_cue` is the only natural language regeneration may copy into the index** (since v0.12.0). Every entry's frontmatter carries an `index_cue` — a budget-checked single sentence (prescriptive: scenario + constraint; descriptive: shortest current-state fact). Step 8 regeneration reads frontmatter only, never the old `index.md`, and refuses to write the file if any entry exceeds the hard budget (cue 100 code points, 5 keywords, 220 code points per rendered line, 32,000 code points whole file — all budgets are Unicode code points, never bytes) — this is a write failure to be fixed by shortening or splitting the entry, not a truncation trigger. Two new capture gates (Step 0.5 scenario-behavior atomicity, Step 0.6 world-state vs behavioral-constraint) and a tightened Step 5 strengthening comparison (same scenario + same constraint, not just same topic) exist to keep entries from drifting into the mixed, budget-busting shape that motivated this version — a single `lesson` entry that had absorbed two unrelated scenario→constraint mappings across 179 lines and grown the whole project-scope index to 57.8k characters.
- **A field declared mandatory is not enforced until a step refuses on its absence** (since v0.12.1). v0.12.0 declared `index_cue` mandatory and then checked only its *budget*, never its *presence* — and a missing field trivially satisfies every budget, so the validation passed a notebook where 28 of 30 entries had no cue at all (two had no conforming frontmatter; one had no frontmatter block whatsoever). `action=audit` was equally blind: its Applicability and Atomicity checks read `index_cue` while presupposing it exists, so they silently evaluated nothing and reported nothing. Hence two standing invariants: **schema presence is validated before budgets** (Step 8 and audit alike), and **a check that could not run must report `blocked`, never pass** — an audit must state how many entries each check actually examined, because a count of flags raised says nothing about coverage. Corollary specific to Step 8: never synthesize a missing `index_cue` from the entry body. Step 8 doesn't read bodies by design; improvising one there produces well-formed-looking output that smuggles back exactly the unbudgeted prose the field exists to exclude.

- **Every budget must name the artifact it governs** (since v0.12.2). v0.12.0's budget table listed numbers without saying whether each bounds the stored entry, the rendered index line, or the finished file — and four defects came out of that one omission: a missing presence check, an undefined unit (characters vs code points), undocumented whole-file semantics, and a keyword row that contradicted the render step. The keyword case is the clearest: Step 8 truncates `keywords` to the first 5 at render time, so a stored count of 22 costs the index nothing — yet the table read as a hard budget on the stored count, which would have forced deleting real search terms from 26 of 29 entries to produce a byte-identical index. **Truncation at render is the precondition for the line budget being satisfiable** (all-keywords rendering puts 12 of 30 lines over 220; first-5 puts 0 over), not an alternative to it. When adding or changing any budget, state its artifact and its unit, and check it can actually be violated — a row that cannot fail is documentation, and a row that always fails is a deadlock.
